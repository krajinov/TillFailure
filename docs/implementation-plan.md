...
- **Entitlement acceptance:** Milestone 3 passed isolated [catalog lifecycle tests](testing-strategy.md#planned-system-catalog-authorization-tests) for multi-workspace retries, first/final contribution, workspace suspension/restoration, revision-checked receipt-backed account enable/disable with stale/out-of-order rejection and strict enable-time count validation, and account plus workspace membership-cap enforcement on activation and restoration. Milestone 4 must integrate the same bounded lifecycle fan-out from [security](firestore-security.md#system-catalog-entitlement-lifecycle); a delayed trigger/claim is insufficient, and identity that cannot be represented by the raw `users/{uid}` path scheme must be handled by the Milestone 4 identity decision rather than silently retargeted. Workspace memberships remain role authority.
