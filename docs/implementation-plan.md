...
- **Architecture proof gate:** Milestone 3 proved deterministic empty-range booking contention, one immutable `utcBucket` lock schema preserved through reschedule/replay, and fixed-path catalog entitlement Rules/atomic lifecycle primitives from [testing-strategy.md](testing-strategy.md#planned-booking-contention-tests), including configured transaction/write caps and workspace-roster cap enforcement on activation. Complete scheduling implementation and approval of final product timing/cap values remain Milestone 10.
