...
| Resolved 2026-09-20 | Exact TypeScript 5 patch and lockfile after Node 22-compatible Functions compile test? Engineering | 3 | TypeScript 5.9.3, Functions 7.4.0, Admin 14.4.0, CLI 15.30.2 and exact lockfile compile/run. Host Node was 25.9.0, so a Node 22 host run remains desirable but does not change the locked engine. |
...
| P0 | Which encryption/data-protection and backup policy makes the passed UID-partitioned atomic-file envelope acceptable on both targets? Security/engineering | 3/8/12 | Atomic restart/isolation passed; encryption at rest, key rotation, backup exclusion and secure erase are not proven. Do not store production private records yet. |
