...
The Node 22 emulator backend passed 84/84 tests on two consecutive clean runs, including a fail-closed lifecycle `schemaVersion` requirement on every path that publishes catalog access (account enable, membership activation, workspace restoration) while deactivation stays possible for missing, malformed, or unsupported versions, Rules-integrated denial for schema-rejected enables, revision-checked, receipt-backed account enable/disable commands (stale enable after a newer disable and stale disable after a newer enable rejected, caller/payload binding, exactly-once lifecycle-revision advance, idempotent replay, exactly one winner for two concurrent opposite commands on the same revision, and unchanged state with no receipt on rejection), strict enable-time entitlement-count validation without coercion of strings, decimals, booleans, nulls, arrays, or objects, security-first disable that always removes catalog authorization while preserving malformed forensic counts, Rules-integrated denial for rejected malformed and above-cap enables and after a legitimate disable, separate relationship-roster and catalog-contribution workspace counters with roster-cap enforcement for active and suspended workspaces, pre-delta validation of every existing entitlement count during suspension/restoration, atomic rejection that preserves denied Rules authorization, fail-closed counter-drift detection, per-membership identity/schema/role/contribution/lifecycle validation during suspension and restoration with unchanged workspace, membership, entitlement, authorization, and receipt state on rejection plus Rules-integrated denial of indirect catalog access through a malformed membership, pre-delta validation of a stored membership in the direct membership command with atomic rejection of a missing or non-boolean contribution flag plus identity/schema/role/status/revision defects and a valid revoke/restore cycle preserved, the documented own-account/workspace assignment-header collection query authorized for its owner while cross-account and ineligible listing stays denied, partial deactivation of a schema-damaged account or entitlement (revoking one of two contributions and suspending one workspace while a contribution remains elsewhere committing with the stored status preserved and Rules-denied reads unchanged, while additions and restorations on the same records still reject atomically and a drifted inactive status is never activated by a removal, nor by a zero contribution delta such as revoking an active relationship in a suspended workspace), single-segment assignment path-ID validation (every user/workspace/assignment/predecessor/workout/exercise/plan ID rejected before any reference or transaction, while punctuated UIDs and IDs still materialize at their exact paths), contribution increases gated on a schema-current recognized workspace while removals, neutral changes, and suspension stay possible, and owner Storage deletes authorized separately from create/update validation (cross-account, foreign-path, and malformed-path deletes denied), participant identifiers validated for the complete representable Firebase UID domain (dotted/email-shaped booking, reschedule, and cancellation with caller-bound receipts; separator injection rejected before Firestore access), callable-level participant authorization with caller-bound receipts, revision-checked single-successor assignment replacement with atomic retirement of both predecessor representations, guarded live-to-terminal assignment closure, plan-to-snapshot reference integrity, and immutable `utcBucket` lock identity across booking/reschedule/replay, membership tenant-identity validation for the caller and both participants across booking, reschedule, and cancellation (a path-correct membership naming another tenant, or a stored `userId` disagreeing with its path, rejected with the appointment, every lock, and every receipt unchanged), and lock cleanup that releases capacity locks only for an explicitly recognized terminal appointment status (missing, malformed, unknown, not-yet-implemented, transient, tenant-mismatched, unknown-schema, and missing-appointment targets failing closed with locks and receipts byte-identical and a conflicting booking still refused). The seventh correction changed only backend TypeScript/tests and the Apple recovery write sequence, so the signed Swift harness and the Xcode Debug build were rerun while Android and shared-Kotlin results are retained: Android host tests 63/63, Android connected tests 66/66 on API 34 (common contract tests plus the native adapter and Keystore persistence suite), iOS simulator shared tests 63/63 (shared Kotlin unchanged, not rerun), and 97 freshly asserted checks in the signed Swift debug harness (44 recovery + 53 Firebase, including the new pre-commit metadata-failure injection) covering Auth, read/write/rejection, cache/pending metadata transitions, transaction, deliberately stalled cancellation and timeout, exactly-once late-completion/epoch fencing, real background/foreground delivery, terminate/clear, one-shot registry lifecycle/leak checks, the canonical counter-value contract (numeric-string, integral, missing, negative, malformed, decimal, whitespace, boolean, collection, and overflow cases), and the complete Firebase UID domain for recovery partitions (email-shaped, punctuated, Unicode, maximum-length, traversal-safe, distinct-partition, empty/overlength rejection, and wrong-UID decryption refusal). Actual Android Keystore and Apple Keychain/CryptoKit recovery implementations passed encrypted restart, UID isolation, tamper/key-loss, backup-exclusion and atomic-replacement checks; the Apple harness additionally passed the new pre-commit metadata-failure, temporary-cleanup, and post-failure recovery checks in this round, while the Android results are retained from the fourth round. The eighth correction changed only backend TypeScript and tests, so all Android, iOS, shared-Kotlin, signed native harness, and Xcode build results are retained without rerun. The ninth correction changed only backend TypeScript, Rules, tests, and documentation, so every native and Xcode result above is likewise retained without rerun. The tenth correction changed backend TypeScript, tests, and documentation only (no Rules change), so every native and Xcode result above is likewise retained without rerun. The eleventh correction also changed backend TypeScript, tests, and documentation only (no Rules change), so every native and Xcode result above is likewise retained without rerun. The twelfth correction changed backend TypeScript, the Storage rules, tests, and documentation, so every native and Xcode result above is likewise retained without rerun. The thirteenth correction changed the shared Firebase error-code contract, the Android adapter with its new app-generation registry and device-test fixture, and the Swift bridge/harness with documentation: Android host tests 63/63, Android connected tests 67/67 on API 34 (including the new same-process termination/recreation regression), iOS simulator shared tests 63/63, the signed Xcode Debug build, and 107 freshly asserted checks in the signed Swift debug harness (43 recovery + 64 Firebase, including the same-process recreation regressions with retired-client denial and retired-epoch fencing) were rerun, while the Node 22 backend/Rules suite (80/80) and the visual evidence are retained because no backend, Rules, or UI source changed. The fourteenth correction changed only backend TypeScript (`booking.ts` membership tenant-identity validation on every booking-path membership read plus lock cleanup restricted to an explicitly recognized terminal appointment status, and the `index.ts` authorization failure mapping), its emulator tests, and documentation, so the Node 22 backend/Rules suite was rerun from a clean start twice (84/84 both runs) while every Android, iOS, shared-Kotlin, signed harness, and Xcode result above is retained without rerun. Native Storage process recovery is deferred to Milestone 9 or 11 and is not part of Milestone 3 acceptance. Full commands and attribution are in [Milestone 3 report](milestones/milestone-3-report.md).
...
# Native iOS opt-in harness build; substitute an available simulator UDID. The harness reads the
# Keychain, so it needs a real codesigned build (ad-hoc is enough): with CODE_SIGNING_ALLOWED=NO the
# linker-only signature leaves the Keychain unavailable, Auth fails with its keychain error, and the
# recovery bridge reports LOCKED.
xcodebuild -project iosApp/iosApp.xcodeproj -scheme iosApp -configuration Debug -destination 'platform=iOS Simulator,id=<SIMULATOR_UDID>' CODE_SIGN_IDENTITY="-" CODE_SIGN_STYLE=Manual CODE_SIGNING_REQUIRED=YES build
# Install the build, launch it with the opt-in flag, and drive background/foreground so its probes run.
SIMCTL_CHILD_TILLFAILURE_FIREBASE_SPIKE=1 xcrun simctl launch --console-pty <SIMULATOR_UDID> com.delminiusapps.tillfailure.TillFailure
```
