...
The Node 22 emulator backend passed 43/43 tests, including callable-level participant authorization with caller-bound receipts, revision-checked single-successor assignment replacement with atomic retirement of both predecessor representations, guarded live-to-terminal assignment closure, plan-to-snapshot reference integrity, workspace-roster cap enforcement on membership activation serialized through the expected workspace revision, and immutable `utcBucket` lock identity across booking/reschedule/replay; Android host tests passed 57/57; Android connected tests passed 58/58; iOS simulator shared tests passed 57/57; and the signed Swift debug harness passed Auth, read/write/rejection, cache/pending metadata transitions, transaction, deliberately stalled cancellation and timeout, exactly-once late-completion/epoch fencing, real background/foreground delivery, terminate/clear, and one-shot registry lifecycle/leak checks. Actual Android Keystore and Apple Keychain/CryptoKit recovery implementations passed encrypted restart, UID isolation, tamper/key-loss, backup-exclusion and atomic-replacement checks. Native Storage process recovery is deferred to Milestone 9 or 11 and is not part of Milestone 3 acceptance. Full commands and attribution are in [Milestone 3 report](milestones/milestone-3-report.md).
